...
function(declare, lang, array, html, query, topic, on, keys, domConstruct, domGeometry,
  all, when, Tooltip, WidgetManager, PanelManager, utils, LoadingShelter, BaseLayoutManager) {
  ...
  clazz = declare([BaseLayoutManager], {
    ...
    _setTabindex: function(appConfig){
      ...
      var tabIndex = 1;
      this._initSkipLinks(rtoWindowElements, tabIndex + 100);
      ...
      //set trap node for resolving the final links that tabindex equal 0.
      var trapLinkNode = query('#trapLinkNode')[0];
      this.own(on(trapLinkNode, 'focus', lang.hitch(this, function() {
        var link = query('#skipContainer a')[0]; //focus first skip node
        link.focus();
      })));
      ...
      utils.addTooltipByDomNode(Tooltip, this.map.container, window.jimuNls.common.mapArea);
    ...
    _initSkipLinks: function(rtoWindowElements, tabindex){
      var skipNls = window.jimuNls.skips;
      //{name: string}
      var skipStrs = {
        'HeaderController': skipNls.headerController,
        'Map': skipNls.map,
        'AttributeTable': skipNls.attributeTable
      };
      var links = '';
      ...
      rtoWindowElements.forEach(function(e){
        if(e.name !== 'Splash' && !e.inPanel){
          var linkName;
          if(e.panel && e.widgets){//side panel that's related to browser for some themes
            if(e.widgets.length === 0){
              return; //continue
            }else{
              linkName = e.widgets[0].label; //using 'Skip to side panel "widgetName"' in next release
            }
          }else{
            linkName = skipStrs[e.name];
            linkName = linkName ? linkName : e.name; //like search widget when it's on the right side of header
          }
          links += '<li><a role="link" id="skip_' + e.id + '" aria-hidden="true" tabindex="' + tabindex + '">' +
          linkName + '</a></li>';
        }
      });
      links += '<li><a role="link" id="skip_map" aria-hidden="true" tabindex="' + tabindex + '">' +
       skipStrs.Map + '</a></li>';
      ...
      this.own(on(linkNodes, 'keydown', lang.hitch(this, function(evt) {
        ...
        else if(!evt.target.parentNode.previousSibling && evt.shiftKey && evt.keyCode === keys.TAB){
          evt.preventDefault();
        }
      })));
    },
