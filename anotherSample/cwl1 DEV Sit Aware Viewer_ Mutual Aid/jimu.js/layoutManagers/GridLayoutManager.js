  ...
  clazz = declare([BaseLayoutManager], {
    ...
    _setTabindex: function(appConfig){
      ...
      if(!splashWidget){
        ...
        html.setAttr(skipContainer , 'tabindex', '1');
        html.setAttr(skipContainer , 'aria-label', '');
        this.own(on(skipContainer, 'focus', lang.hitch(this, function() {
          var link = query('a', skipContainer)[0]; //focus first skip node
          link.focus();
        })));
      }
      ...
      this.own(on(trapLinkNode, 'focus', lang.hitch(this, function(evt) {
        console.log('trapLinkNode...focus');
        var splashW = this.widgetManager.getWidgetsByName('Splash')[0];
        if(splashW && html.getStyle(splashW.domNode, 'display') !== 'none'){
          evt.preventDefault();
          var splashFirstNode = query('.' + utils.firstFocusNodeClass, splashW.domNode)[0];
