    ...
    var so = declare([_WidgetBase, _TemplatedMixin, _WidgetsInTemplateMixin], {
      ...
      _updateResUrls: function(param) {
        ...
        if (this.setLayerVisibility.checked) {
          this.resultUrl = shareUtils.addQueryParamToUrl(this.resultUrl, "showlayers",
            this._getLayersVisibility(), true);
        }
      },

      _getLayersVisibility: function(){
        var layers = [];
        var visibility = this.layerInfosObj.getSimplificationState();
        if (visibility && visibility.showLayers && visibility.showLayers.join) {
          layers = visibility.showLayers.join(";");
        }
